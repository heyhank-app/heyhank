...
describe("ContainerManager git identity seeding from host .gitconfig", () => {
  ...
  it("copies user.name and user.email from /heyhank-host-gitconfig into container global config", () => {
    // The host .gitconfig is mounted read-only at /heyhank-host-gitconfig.
    ...
    const identityCmd = commands.find((cmd) => cmd.includes("heyhank-host-gitconfig"));
    expect(identityCmd).toBeDefined();
    // It should use git config -f to read from the mounted file
    expect(identityCmd).toContain("git config -f /heyhank-host-gitconfig user.name");
    expect(identityCmd).toContain("git config -f /heyhank-host-gitconfig user.email");
  ...
  it("disables gpgsign in writable global config (not the read-only mount)", () => {
    // With the host .gitconfig mounted at /heyhank-host-gitconfig instead
...
describe("ContainerManager Codex file seeding", () => {
  ...
  it("seeds Codex auth files when /heyhank-host-codex is available", () => {
    ...
    expect(commands.some((cmd) =>
      cmd.includes("/heyhank-host-codex") && cmd.includes("/root/.codex"),
    )).toBe(true);
  });
  ...
  it("copies auth.json, config.toml, and directory seeds for Codex", () => {
    ...
    const commands = mockExecSync.mock.calls.map((call) => String(call[0] ?? ""));
    const seedCmd = commands.find((cmd) => cmd.includes("heyhank-host-codex"));
...
describe("ContainerManager removeContainer", () => {
  ...
  it("removes container and volume from docker and internal map", () => {
    ...
    manager.restoreContainer("sess-1", {
      containerId: "abc123", name: "heyhank-abc", image: "node:22",
      portMappings: [], hostCwd: "/tmp", containerCwd: "/workspace",
      state: "running", volumeName: "heyhank-ws-abc",
    });
...
describe("ContainerManager.getRegistryImage", () => {
  it("returns registry path for heyhank:latest", () => {
    const result = ContainerManager.getRegistryImage("heyhank:latest");
    expect(result).toContain("stangirard/heyhank:latest");
  });
...
describe("ContainerManager createContainer", () => {
  ...
  it("creates a container with volume, ports, and auth seeding", () => {
    ...
    mockExecSync.mockImplementation((...args: unknown[]) => {
      callCount++;
      const cmd = String(args[0] ?? "");
      if (cmd.includes("docker volume create")) return "heyhank-ws-test1234";
    ...
    expect(info.volumeName).toBe("heyhank-ws-test1234");
  });
  ...
  it("mounts host .gitconfig when it exists", () => {
    ...
    expect(createCmd).toContain("heyhank-host-gitconfig");
  });
});
...
describe("ContainerManager seedAuthFiles via startContainer", () => {
  ...
  it("copies auth files from /heyhank-host-claude to /root/.claude", () => {
    mockExecSync.mockReturnValue("");
    const manager = new ContainerManager();
    manager.startContainer("abc123");

    const cmds = mockExecSync.mock.calls.map((c) => String(c[0] ?? ""));
    // seedAuthFiles runs a docker exec with heyhank-host-claude
    expect(cmds.some((c) => c.includes("heyhank-host-claude") && c.includes("/root/.claude"))).toBe(true);
  });
});
