...
describe("authenticateManagedWebSocket", () => {
  const savedSecret = process.env.HEYHANK_AUTH_SECRET;
  ...
  afterEach(() => {
    if (savedSecret === undefined) delete process.env.HEYHANK_AUTH_SECRET;
    else process.env.HEYHANK_AUTH_SECRET = savedSecret;
  });
